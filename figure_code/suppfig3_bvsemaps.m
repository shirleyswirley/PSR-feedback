...
data_path = '/graid1/shirlleu/PSRfeedback/data/globalbetavsexportmaps_tempregress/';
...
% beta vs. export maps (bvse)
eration = length(eratio_algs);
nppn = length(npp_algs);
timen = 160; % num of months in time series
load([data_path 'betavsexportn1tempregmo_VGPMVGPMECbPMcompiled.mat'],'lat1','lon1')
bvse = nan(length(lat1),length(lon1),eration,nppn);

% - Load b vs e maps + populate bvse
for ieratio = 1:eration
    for inpp = 1:nppn
        load([data_path npp_algs{inpp} 'based/' betanamearray{ibeta} 'betavs' eratio_algs{ieratio} 'exportn1tempregmo.mat'],'betavsetempregresscoeffmapzeros');
...
for ibeta = 1:betan
    for ieratio = 1:eration
        for inpp = 1:nppn
            load([fnamebase npp_algs{inpp} 'based/' betanamearray{ibeta} 'betavs' eratio_algs{ieratio} 'exportn1tempregmo.mat'],'betavsetempregresscoeffmapzeros');
            % GET RID OF CRAZY OUTLIERS
            betavsetempregresscoeffmapzeros(abs(betavsetempregresscoeffmapzeros)>=5)=nan;
            length(find(abs(betavsetempregresscoeffmapzeros)>=5)) % displays how many outliers there were in each beta vs export map
            bvse(:,:,ibeta,ieratio,inpp) = betavsetempregresscoeffmapzeros;
...
for ibeta = 1:betan
    ...
    for inpp = 1:nppn
        for ieratio = 1:eration
            ...
            title([betanamearray{ibeta} 'beta vs.' 10 npp_algs{inpp} ' NPP+' eratio_algs{ieratio} ' e-ratio']);
            isp = isp+1;
        end
    end
end
...
%         title(['All beta vs.' 10 npp_algs{inpp} ' npp+' eratio_algs{ieratio} ' e-ratio reg wts']);
%         isp = isp+1;
%     end
% end
...
for ibeta = 1:betan
    ...
    exportlabels = {[npp_algs{1} '+' eratio_algs_sh{1}],[npp_algs{2} '+' eratio_algs_sh{1}],[npp_algs{3} '+' eratio_algs_sh{1}],...
        [npp_algs{1} '+' eratio_algs_sh{2}],[npp_algs{2} '+' eratio_algs_sh{2}],[npp_algs{3} '+' eratio_algs_sh{2}],...
        [npp_algs{1} '+' eratio_algs_sh{3}],[npp_algs{2} '+' eratio_algs_sh{3}],[npp_algs{3} '+' eratio_algs_sh{3}]};
    caxis([0 9]);
